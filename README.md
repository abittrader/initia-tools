...
```
### 7. Add seeds and peers to the config.toml - Always Check New Peers in Discord
```bash
...
```
### 9. Configure prunning to save storage (Optional)- AT first for better sync, its better to not enable it
```bash
sed -i \
...
```
### 11. Enable indexer (Optional)-AT first for better sync, its better to not enable it
```bash
